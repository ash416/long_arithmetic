#include "BigIntLittle2.h"

CBigIntLittle2::CBigIntLittle2()
{
}

CBigIntLittle2::CBigIntLittle2(std::string str)
{
	int leng = str.length();
	if (str[0] == '-') {
		this->sign = false;
		this->size = leng - 1;
	}
	else {
		this->sign = true;
		this->size = leng;
	}
	this->buf = (int*)malloc(size * sizeof(int));
	for (int i = 0; i < size; i++)
		*(buf + i) = int(str[leng - i - 1] - '0');
	this->num = &this->buf[0];
}

int & CBigIntLittle2::operator[](const int i)
{
	return num[i];
}

int CBigIntLittle2::operator[](const int i) const
{
	return num[i];
}

bool operator==(const CBigIntLittle2 & a, const CBigIntLittle2 & b)
{
	if (a.size != b.size || a.sign != b.sign)
		return false;
	else {
		for (int i = 0; i < a.size; i++)
			if (a[i] != b[i])
				return false;
		return true;
	}
}

bool operator<(const CBigIntLittle2 & a, const CBigIntLittle2 &b) {
	if (a.sign == b.sign) {
		if (a.size != b.size)
			return a.size > b.size ? !a.sign : a.sign;
		else { // a.size() == b.size()
			for (int i = a.size - 1; i >= 0; i--) {
				if (a[i] == b[i])
					continue;
				else
					return a.sign ? (a[i] < b[i]) : !(a[i] < b[i]);
			}
			return false;
		}
	}
	else {
		return !a.sign;
	}
}

bool operator>(const CBigIntLittle2 & a, const CBigIntLittle2 &b) {
	if (a.sign == b.sign) {
		if (a.size != b.size)
			return a.size > b.size ? a.sign : !a.sign;
		else { // a.size() == b.size()
			for (int i = a.size - 1; i >= 0; i--) {
				if (a[i] == b[i])
					continue;
				else
					return !a.sign ? (a[i] < b[i]) : !(a[i] < b[i]);
			}
			return false;
		}
	}
	else {
		return a.sign;
	}
}

bool operator >=(const CBigIntLittle2 &a, const CBigIntLittle2 &b) {
	return(!(a < b));
}

bool operator <=(const CBigIntLittle2 &a, const CBigIntLittle2 &b) {
	return (!(a > b));
}

bool operator !=(const CBigIntLittle2 &a, const CBigIntLittle2 &b) {
	return (!(a == b));
}

CBigIntLittle2 &CBigIntLittle2::operator =(const CBigIntLittle2 &num) {
	free(this->buf);
	this->buf = (int*)malloc(num.size * sizeof(int));
	for (int i = 0; i < num.size; i++)
		this->buf[i] = num[i];
	this->sign = num.sign;
	this->size = num.size;
	this->num = &this->buf[0];
	return *this;
}

CBigIntLittle2 operator+(const CBigIntLittle2 & num1, const CBigIntLittle2 & num2)
{
	CBigIntLittle2 sum;
	int carry = 0;
	if (num1.sign == num2.sign) {
		sum.size = (num1.size > num2.size ? num1.size : num2.size) + 1;
		sum.buf = (int*)malloc(sum.size * sizeof(int));
		sum.num = &sum.buf[0];
		for (int i = 0; i < sum.size - 1; i++) {
			int temp;
			if (i >= num1.size)
				temp = num2[i] + carry;
			else
				if (i >= num2.size)
					temp = num1[i] + carry;
				else
					temp = num1[i] + num2[i] + carry;
			sum[i] = temp % 10;
			carry = temp / 10;
		}
		if (carry > 0) {
			sum[sum.size - 1] = carry;
		}
		else
			sum.size--;
		sum.sign = num1.sign;
	}
	else {
		bool num1_is_less;
		if (num1.size != num2.size)
			num1_is_less = num1.size > num2.size ? false : true;
		else {
			int i;
			for (i = num1.size - 1; i >= 0; i--) {
				if (num1[i] == num2[i])
					continue;
				else {
					num1_is_less = (num1[i] < num2[i]);
					break;
				}
			}
			if (i < 0)
				num1_is_less = false;
		}
		if (num1_is_less) {
			sum.size = num2.size;
			sum.buf = (int*)malloc(sum.size * sizeof(int));
			sum.num = &sum.buf[0];
			for (int i = 0; i < num2.size; i++) {
				if (i >= num1.size) {
					sum[i] = (num2[i] - carry + 10) % 10;
					carry = (num2[i] - carry) >= 0 ? 0 : 1;
					continue;
				}
				sum[i] = (num2[i] - carry - num1[i] + 10) % 10;
				carry = (num2[i] - carry >= num1[i] ? 0 : 1);
			}
		}
		else {
			sum.size = num1.size;
			sum.buf = (int*)malloc(sum.size * sizeof(int));
			sum.num = &sum.buf[0];
			for (int i = 0; i < num1.size; i++) {
				if (i >= num2.size) {
					sum[i] = (num1[i] - carry + 10) % 10;
					carry = (num1[i] - carry) >= 0 ? 0 : 1;
					continue;
				}
				sum[i] = (num1[i] - carry - num2[i] + 10) % 10;
				carry = (num1[i] - carry >= num2[i] ? 0 : 1);
			}
		}
		sum.sign = num1_is_less ? num2.sign : num1.sign;
		while (sum.size > 1 && sum[sum.size - 1] == 0) {
			sum.size--;
		}
	}
	return sum;
}

CBigIntLittle2 operator-(const CBigIntLittle2 & num1, CBigIntLittle2 & num2)
{
	num2.sign = false;
	return num1 + num2;
}

CBigIntLittle2 operator*(const CBigIntLittle2 &num1, const CBigIntLittle2 &num2) {
	CBigIntLittle2 result;
	result.size = num1.size + num2.size;
	result.buf = (int*)malloc(result.size * sizeof(int));
	result.num = &result.buf[0];
	for (int i = 0; i < result.size; i++)
		result[i] = 0;
	for (int i = 0; i < num2.size; ++i) {
		if (num2[i] != 0) {
			int carry = 0;
			for (int j = 0; j < num1.size; ++j) {
				int temp = num1[j] * num2[i] + result[i + j] + carry;
				result[i + j] = temp % 10;
				carry = temp / 10;
			}
			result[i + num1.size] = carry;
		}
	}
	while (result.size > 1 && result[result.size - 1] == 0) {
		result.size--;
	}
	result.sign = num1.sign == num2.sign ? true : false;
	return result;
}

CBigIntLittle2 operator*(const CBigIntLittle2 & a, const int & c) {
	if (c == 0)
		return CBigIntLittle2("0");
	CBigIntLittle2 result;
	int b = c;
	result.sign = (a.sign && b > 0) || (!a.sign && b < 0) ? true : false;
	if (b < 0) b *= (-1);
	int temp, carry = 0;
	int sizeB = 0;
	for (int i = b; i > 0; i = i / 10, sizeB++);
	result.size = a.size + sizeB;
	result.buf = (int*)malloc(result.size * sizeof(int));
	result.num = &result.buf[0];
	for (int i = 0; i < result.size; i++)
		result[i] = 0;
	int i;
	for (i = 0; i < a.size; i++) {
		temp = a[i] * b + carry;
		carry = temp / 10;
		result[i] = temp - carry * 10;
	}
	while (carry > 0) {
		result[i++] = carry % 10;
		carry /= 10;
	}
	return result;
}

CBigIntLittle2 operator/(const CBigIntLittle2 & a, const CBigIntLittle2 & b)
{
	if (a.size < b.size)
		return CBigIntLittle2("0");
	if (a.size == b.size) {
		for (int i = a.size - 1; i >= 0; i--) {
			if (a[i] == b[i])
				continue;
			else
				if (a[i] < b[i])
					return CBigIntLittle2("0");
				else break;
		}
	}
	CBigIntLittle2 q;
	q.sign = a.sign == b.sign ? true : false;
	CBigIntLittle2 copyA;
	copyA.size = a.size + 1;
	copyA.buf = (int*)malloc(copyA.size * sizeof(int));
	copyA.num = &copyA.buf[0];
	for (int i = 0; i < copyA.size - 1; i++)
		copyA[i] = a[i];
	copyA[copyA.size - 1] = 0;
	int shift = 0;
	CBigIntLittle2 copyB;
	copyB = b;

	short scale;
	short qGuess, r;
	short borrow, carry;
	int n = b.size, m = a.size - b.size;
	scale = 10 / (b[n - 1] + 1);
	if (scale > 1) {
		copyA = copyA * scale;
		if (copyA[copyA.size - 1] == 0) {
			copyA.size--;
		}
		copyB = copyB * scale;
		if (copyB[copyB.size - 1] == 0) {
			copyB.size--;
		}
	}
	q.size = m + 1;
	q.buf = (int*)malloc(q.size * sizeof(int));
	q.num = &q.buf[0];
	int q_ind, a_ind;
	for (q_ind = m, a_ind = n + q_ind; q_ind >= 0; q_ind--, a_ind--) {
		int temp = (copyA[a_ind] * 10 + copyA[a_ind - 1]);
		qGuess = temp / copyB[n - 1];
		r = temp % copyB[n - 1];
		while (r < 10) {
			if (copyB[n - 2] * qGuess > r * 10 + copyA[a_ind - 2] || qGuess == 10) {
				qGuess--;
				r += copyB[n - 1];
			}
			else break;
		}
		carry = 0;
		borrow = 0;
		int *aShift = copyA.buf + q_ind;

		int temp1, temp2;
		for (int i = 0; i < n; i++) {
			temp1 = copyB[i] * qGuess + carry;
			carry = temp1 / 10;
			temp1 -= carry * 10;
			temp2 = aShift[i] - temp1 + borrow;
			if (temp2 < 0) {
				aShift[i] = temp2 + 10;
				borrow = -1;
			}
			else {
				aShift[i] = temp2;
				borrow = 0;
			}
		}

		temp2 = aShift[n] - carry + borrow;
		if (temp2 < 0) {
			aShift[n] = temp2 + 10;
			borrow = -1;
		}
		else {
			aShift[n] = temp2;
			borrow = 0;
		}

		if (borrow == 0) {
			if (q_ind + shift == m && qGuess == 0) {
				shift++;
				continue;
			}
			q[q_ind + shift] = qGuess;
		}
		else {
			if (q_ind + shift == m && qGuess - 1 == 0) {
				shift++;
			}
			else {
				q[q_ind + shift] = qGuess - 1;
			}
			carry = 0;
			for (int i = 0; i < n; i++) {
				temp = aShift[i] + copyB[i] + carry;
				if (temp >= 10) {
					aShift[i] = temp - 10;
					carry = 1;
				}
				else {
					aShift[i] = temp;
					carry = 0;
				}
			}
			aShift[n] += carry - 10;
		}
	}
	copyA.deleteNumber();
	copyB.deleteNumber();
	if (shift != 0) {
		m -= shift;
		q.num = &q.buf[shift];
	}
	q.size = m + 1;

	return q;
}

std::istream &operator >> (std::istream &stream, CBigIntLittle2 &num) {
	std::string str;
	stream >> str;
	CBigIntLittle2 _int(str);
	num = _int;
	return stream;
}
std::ostream &operator<<(std::ostream &stream, const CBigIntLittle2 &num) {
	if (!num.sign)
		stream << '-';
	for (int i = num.size - 1; i >= 0; i--) {
		stream << num[i];
	}
	return stream;
}
