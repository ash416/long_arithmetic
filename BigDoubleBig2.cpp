...
CBigDoubleBig2::CBigDoubleBig2(std::string str)
{
	...
	this->int_part = (CBigDoubleBig2::AtomicT*)malloc(this->int_size);
	this->frac_part = (CBigDoubleBig2::AtomicT*)malloc(this->frac_size);
	size_t j = 0;
...
CBigDoubleBig2::AtomicT & CBigDoubleBig2::operator[](const size_t i)
{
	if (i >= this->int_size)
...
CBigDoubleBig2::AtomicT CBigDoubleBig2::operator[](const size_t i) const
{
	if (i >= this->int_size)
		return this->frac_part[i - this->int_size];
	else
...
{
	CBigDoubleBig2::AtomicT carry = 0;
	sum.int_size = std::max(small.int_size, big.int_size);
	sum.frac_size = std::max(small.frac_size, big.frac_size);
	sum.int_part = (CBigDoubleBig2::AtomicT*)malloc(sum.int_size);
	sum.frac_part = (CBigDoubleBig2::AtomicT*)malloc(sum.frac_size);
	size_t sum_index = sum.frac_size - 1;
	for (size_t i = sum.frac_size - 1; i >= 0; i--) {
		if (i >= small.frac_size) sum.frac_part[sum_index--] = big.frac_part[i];
			...
			CBigDoubleBig2::AtomicT temp = big.frac_part[i] - small.frac_part[i] - carry + 10;
			sum.frac_part[sum_index--] = temp % 10;
			carry = big.frac_part[i] - carry >= small.frac_part[i] ? 0 : 1;
		}
	}
	sum_index = sum.int_size - 1;
	for (size_t i = small.int_size - 1, j = big.int_size - 1; sum_index >= 0; i--, j--, sum_index--) {
		if (i < 0) {
...
CBigDoubleBig2 operator+(const CBigDoubleBig2 & num1, const CBigDoubleBig2 & num2)
{
	...
	if (num1.sign == num2.sign) {
		...
		sum.int_part = (CBigDoubleBig2::AtomicT*)malloc(sum.int_size);
		sum.frac_part = (CBigDoubleBig2::AtomicT*)malloc(sum.frac_size);
		int sum_index = (int)sum.frac_size - 1;
		for (int i = (int)sum.frac_size - 1; i >= 0; i--) {
			if (i >= (int)num1.frac_size) sum.frac_part[sum_index--] = num2.frac_part[i];
			else if (i >= (int)num2.frac_size) sum.frac_part[sum_index--] = num1.frac_part[i];
			else {
				CBigDoubleBig2::AtomicT temp = num1.frac_part[i] + num2.frac_part[i] + carry;
				sum.frac_part[sum_index--] = temp % 10;
				carry = temp / 10;
			}
		}
		sum_index = (int)sum.int_size - 1;
		for (int i = (int)num1.int_size - 1, j = (int)num2.int_size - 1; sum_index >= 0; i--, j--, sum_index--) {
			CBigDoubleBig2::AtomicT temp;
			if (i < 0)
		...
		if (carry > 0) {
			sum.int_part = (CBigDoubleBig2::AtomicT*)realloc(sum.int_part, (sum.int_size + 1));
			for (int i = (int)sum.int_size; i > 0; i--)
		...
		if (i > 0) {
			for (size_t j = 0; j < sum.int_size - i + 1; j++)
				sum.int_part[j] = sum.int_part[j + i];
			sum.int_part = (CBigDoubleBig2::AtomicT*)realloc(sum.int_part, sum.int_size);
		}
	}
	return sum;
}
...
CBigDoubleBig2 operator*(const CBigDoubleBig2 &num1, const CBigDoubleBig2 &num2) {
	...
	result.int_part = (CBigDoubleBig2::AtomicT*)malloc(result.int_size);
	result.frac_part = (CBigDoubleBig2::AtomicT*)malloc(result.frac_size);
	...
	for (size_t i = num2.int_size + num2.frac_size - 1; i >= 0; i--) {
		if (num2[i] != 0) {
			CBigDoubleBig2::AtomicT carry = 0;
			for (size_t j = num1.int_size + num1.frac_size - 1; j >= 0; j--) {
				CBigDoubleBig2::AtomicT temp = num1[j] * num2[i] + result[i + j + 1] + carry;
	...
	if (i > 0) {
		for (size_t j = 0; j <= result.int_size - 1; j++) {
			result.int_part[j] = result.int_part[j + i];
		}
		result.int_part = (CBigDoubleBig2::AtomicT*)realloc(result.int_part, result.int_size);
	}
...
CBigDoubleBig2 operator*(const CBigDoubleBig2 & a, const int & c) {
	...
	if (b < 0) b *= (-1);
	CBigDoubleBig2::AtomicT temp, carry = 0;
	size_t sizeB = 0;
...
CBigDoubleBig2 operator/(const CBigDoubleBig2 & a, const CBigDoubleBig2 & b)
{
	...
	size_t accuracy = 3;
	size_t frac_diff = b.frac_size - a.frac_size;
	copyA.int_size = a.int_size + a.frac_size + 1 + (frac_diff > 0 ? frac_diff : 0) + accuracy;
	copyA.int_part = (CBigDoubleBig2::AtomicT*)malloc(copyA.int_size);
	copyA[0] = 0;
	size_t i, j;
	...
	for (i = a.int_size + a.frac_size + 1; i < copyA.int_size; i++)
		copyA[i] = 0;
	size_t a_size = a.int_size + a.frac_size + frac_diff;

	size_t shift = 0;
	CBigDoubleBig2 copyB;
	copyB.int_size = b.int_size + b.frac_size;
	copyB.int_part = (CBigDoubleBig2::AtomicT*)malloc(copyB.int_size);
	for (size_t i = 0; i < copyB.int_size; i++) {
		copyB.int_part[i] = b[i];
	}

	CBigDoubleBig2::AtomicT scale;
	CBigDoubleBig2::AtomicT qGuess, r;
	CBigDoubleBig2::AtomicT borrow, carry;
	...
	CBigDoubleBig2::AtomicT *aShift = copyA.int_part + i;
	CBigDoubleBig2::AtomicT *bShift = copyB.int_part + j;
	if (aShift[0] != 0)
		diff++;
	else
	...
	q.int_part = (CBigDoubleBig2::AtomicT*)malloc(q.int_size);
	q.frac_part = (CBigDoubleBig2::AtomicT*)malloc(q.frac_size);
	...
	for (q_ind = 0, a_ind = 0; q_ind <= m; q_ind++, a_ind++) {
		...
		CBigDoubleBig2::AtomicT temp = (aShift[a_ind] * 10 + aShift[a_ind + 1]);
		...
		CBigDoubleBig2::AtomicT temp1, temp2;
		...
		if (borrow == 0) {
			if (q_ind - shift == 0 && qGuess == 0) {
				if (aShift[1] >= bShift[0]) {
					q.frac_size++;
					q.int_size--;
					q.frac_part = (CBigDoubleBig2::AtomicT*)realloc(q.frac_part, q.frac_size);
					q.int_part = (CBigDoubleBig2::AtomicT *)realloc(q.int_part, q.int_size);
				}
				shift++;
				continue;
			}
...
std::ostream &operator<<(std::ostream &stream, const CBigDoubleBig2 &num) {
	if (!num.sign)
		stream << '-';
	for (size_t i = 0; i < num.int_size; i++) {
		stream << num.int_part[i];
	}
	stream << '.';
	for (size_t i = 0; i < num.frac_size; i++) {
